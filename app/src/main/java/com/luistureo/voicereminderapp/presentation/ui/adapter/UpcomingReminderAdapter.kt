...
class UpcomingReminderAdapter(
    private var items: List<UpcomingReminderListItem>,
    ...
    override fun getItemViewType(position: Int): Int {
        return when (items[position]) {
            is UpcomingReminderListItem.DayHeader -> VIEW_TYPE_HEADER
            is UpcomingReminderListItem.ReminderRow -> VIEW_TYPE_REMINDER
        }
    }
        ...
        return when (viewType) {
            VIEW_TYPE_HEADER -> {
                val view = inflater.inflate(
                    R.layout.item_upcoming_day_header,
                    parent,
                    false
                )
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (val item = items[position]) {
            is UpcomingReminderListItem.DayHeader -> bindHeader(holder as HeaderViewHolder, item)
            is UpcomingReminderListItem.ReminderRow -> bindReminder(
    ...
    fun updateData(newItems: List<UpcomingReminderListItem>) {
        items = newItems
        notifyDataSetChanged()
    }

    private fun bindHeader(holder: HeaderViewHolder, item: UpcomingReminderListItem.DayHeader) {
    ...
    private fun bindReminder(
        ...
        item: UpcomingReminderListItem.ReminderRow
    ) {
        ...
        holder.urgentBadge.text = holder.itemView.context.getString(R.string.calendar_upcoming_urgent_badge)
        holder.card.contentDescription = buildList {
            add(reminder.title)
            reminder.detail.takeIf { it.isNotBlank() }?.let(::add)
            add(holder.date.text.toString())
            add(holder.time.text.toString())
            if (reminder.isUrgent) add(holder.urgentBadge.text.toString())
            if (reminder.isCompleted) {
                add(holder.itemView.context.getString(R.string.calendar_completed_label))
            }
