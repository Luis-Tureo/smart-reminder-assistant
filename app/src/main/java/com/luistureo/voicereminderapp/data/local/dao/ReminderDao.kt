...
interface ReminderDao {
    ...
    @Query("SELECT * FROM reminders WHERE hiddenFromApp = 0 ORDER BY COALESCE(nextTriggerAtEpochMillis, scheduledAtEpochMillis) ASC, id DESC")
    suspend fun getAllReminders(): List<ReminderEntity>

    @Query("SELECT * FROM reminders ORDER BY COALESCE(nextTriggerAtEpochMillis, scheduledAtEpochMillis) ASC, id DESC")
    suspend fun getAllRemindersIncludingHidden(): List<ReminderEntity>
